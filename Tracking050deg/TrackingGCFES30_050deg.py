...
with tides only for 30 days (~2x spring-neap cycle), particle spacing 0.5 degrees
...
def TidalMotionM2S2K1O1_30Days(particle, fieldset, time, dt):
    """
    ...
    """
    if time < 30*24*60*60: # only tides for the first 30 days
        ...
        Vampl_S2_3 = f_S2 * fieldset.VaS2[time + 0.5*dt, lon_2, lat_2, particle.depth] 
        Vpha_S2_3 = V_S2 + u_S2 - fieldset.VgS2[time + 0.5*dt, lon_2, lat_2, particle.depth] 
        Vampl_K1_3 = f_K1 * fieldset.VaK1[time + 0.5*dt, lon_2, lat_2, particle.depth]
        Vpha_K1_3 = V_K1 + u_K1 - fieldset.VgK1[time + 0.5*dt, lon_2, lat_2, particle.depth]
        Vampl_O1_3 = f_O1 * fieldset.VaO1[time + 0.5*dt, lon_2, lat_2, particle.depth]
        Vpha_O1_3 = V_O1 + u_O1 - fieldset.VgO1[time + 0.5*dt, lon_2, lat_2, particle.depth]   
        # Zonal and meridional tidal currents; time + fieldset.t0rel = number of seconds elapsed between t0 and time
        ...
        # New lon + lat
        lon_3, lat_3 = (particle.lon + U3*dt, particle.lat + V3*dt) 
        ...
        # Zonal amplitudes and phaseshifts at particle location and time
        Uampl_M2_4 = f_M2 * fieldset.UaM2[time + dt, lon_3, lat_3, particle.depth]
        Upha_M2_4 = V_M2 + u_M2 - fieldset.UgM2[time + dt, lon_3, lat_3, particle.depth]
        Uampl_S2_4 = f_S2 * fieldset.UaS2[time + dt, lon_3, lat_3, particle.depth]
        Upha_S2_4 = V_S2 + u_S2 - fieldset.UgS2[time + dt, lon_3, lat_3, particle.depth] 
        Uampl_K1_4 = f_K1 * fieldset.UaK1[time + dt, lon_3, lat_3, particle.depth]
        Upha_K1_4 = V_K1 + u_K1 - fieldset.UgK1[time + dt, lon_3, lat_3, particle.depth]
        Uampl_O1_4 = f_O1 * fieldset.UaO1[time + dt, lon_3, lat_3, particle.depth]
        Upha_O1_4 = V_O1 + u_O1 - fieldset.UgO1[time + dt, lon_3, lat_3, particle.depth]   
        # Meridional amplitudes and phaseshifts at particle location and time
        Vampl_M2_4 = f_M2 * fieldset.VaM2[time + dt, lon_3, lat_3, particle.depth]
        Vpha_M2_4 = V_M2 + u_M2 - fieldset.VgM2[time + dt, lon_3, lat_3, particle.depth] 
        Vampl_S2_4 = f_S2 * fieldset.VaS2[time + dt, lon_3, lat_3, particle.depth] 
        Vpha_S2_4 = V_S2 + u_S2 - fieldset.VgS2[time + dt, lon_3, lat_3, particle.depth] 
        Vampl_K1_4 = f_K1 * fieldset.VaK1[time + dt, lon_3, lat_3, particle.depth]
        Vpha_K1_4 = V_K1 + u_K1 - fieldset.VgK1[time + dt, lon_3, lat_3, particle.depth]
        Vampl_O1_4 = f_O1 * fieldset.VaO1[time + dt, lon_3, lat_3, particle.depth]
        Vpha_O1_4 = V_O1 + u_O1 - fieldset.VgO1[time + dt, lon_3, lat_3, particle.depth]   
        # Zonal and meridional tidal currents; time + fieldset.t0rel = number of seconds elapsed between t0 and time
...
totalKernel = AdvectionRK4 + pset.Kernel(TotalDistance) + pset.Kernel(AntiBeach) + pset.Kernel(TidalMotionM2S2K1O1_30Days)
...
outputfile = pset.ParticleFile(name = "/science/projects/oceanparcels/output_data/data_Miriam/Results050deg/Results_TrackingGCFES30_050deg.nc")
